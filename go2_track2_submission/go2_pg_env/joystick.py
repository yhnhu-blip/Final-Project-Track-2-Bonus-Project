...
class Joystick(go2_base.Go2Env):
    ...
    def _student_stage2_sampling_profile(self, current_command: jax.Array) -> tuple[jax.Array, jax.Array, jax.Array]:
        """Stage 2: expanded command distribution covering high-speed + yaw + lateral.

        Uses student_stage2_goal_* values set from course_config.json
        "student_stage2_goal" block — e.g. vx up to 2.5 m/s, vy ±0.2, yaw ±0.8.
        """
        del current_command
