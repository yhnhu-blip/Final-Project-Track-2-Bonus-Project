...
def _patch_jax_deprecated_apis() -> None:
    """Restore APIs removed in JAX 0.10+ that Brax 0.14 still calls."""
    import jax
    if not hasattr(jax, "device_put_replicated"):
        # device_put_replicated(x, devices) ≡ device_put(tree_map(x[None]), devices[0])
        # This preserves the (num_devices, *shape) convention that pmap/_unpmap expect.
        import jax.tree_util
        def _device_put_replicated(x, devices):
            return jax.device_put(
                jax.tree_util.tree_map(lambda y: y[None], x),
                devices[0],
            )
...
def apply_stage_config(env_cfg: Any, ppo_cfg: Any, config: dict[str, Any], stage_name: str) -> None:
    ...
    # Allow top-level tracking_sigma override (default 0.25 is too tight for high-speed commands)
    if "tracking_sigma" in config:
        env_cfg.reward_config.tracking_sigma = float(config["tracking_sigma"])
